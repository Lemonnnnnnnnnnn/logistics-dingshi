...
import { List, InputItem, Modal, Button, Toast, Popover } from 'antd-mobile'
...
class FieldInput extends Component {
  ...
  render () {
    const { field:{ fieldInputType, placeholder, label, extra, prefixCls, editable }, onChange, value } = this.props
        ...
        <List onClick={this.clickInput}>
          <Popover
            ...
            overlay={editable === false?[
              <Popover.Item>{value}</Popover.Item>
            ]:null}
          >
            <InputItem
              ...
              // editable={!modalInput}
              editable={editable}
              extra={<span style={{ color:'rgba(111,127,175,1)', paddingLeft:'7px' }}>{extra}</span>}
              onChange={onChange}
            >{label}
            </InputItem>
          </Popover>
