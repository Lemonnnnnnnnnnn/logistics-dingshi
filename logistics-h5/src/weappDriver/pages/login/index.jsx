...
class Login extends Component {
  ...
  login = ({ userName, password }) => {
    ...
    Toast.loading("", 10)
    this.props.login({ userName, password:encodePassword(password), scope:'DRIVER' })
      .then(()=>{
        setTimeout(()=>this.props.getDictionaries(), 300)
        localStorage.removeItem("driver-showHomeModal");
      })
  ...
  _login = ({ userPhone, smsCode }) => {
    Toast.loading("", 10)
    this.props.login({ userName:userPhone, smsCode, scope:'DRIVER' })
      .then(()=>{
        setTimeout(()=>this.props.getDictionaries(), 300)
        localStorage.removeItem("driver-showHomeModal");
      })
        ...
        <SchemaForm schema={this.schema} trigger={{ showPwd }}>
          <Card>
            <Card.Body>
              ...
              <div style={{ float:'right', fontSize:'14px', color:'#999' }}>
                还没有账号？
                <span onClick={this.registry} style={{ color:'rgba(46,122,245,1)' }}>
                  立即注册
                </span>
              </div>
            </Card.Body>
