...
export default function (AddComponent) {
  ...
  static getDerivedStateFromProps (nextProps, prevState) {
    ...
    renderImgPreview = (file, index) => {
      ...
      const imgUrl = getOssImg(file, previewConfig);
      const { imageStyle = {} } = this.props.field;
      ...
      return (
        <span key={file} styleName='img-preview-wrap'>
          <PopUpImg
            ...
            src={imgUrl}
            style={{ width: '30vw', height: '30vw', margin: '10px 0px 30px 2.5vw', ...imageStyle }}
            isShowImage={this.props.field.addMode !== 'camera'}
          />
          {readOnly ||
          <span onClick={() => this.removeFile(file, index)} styleName='remove-file'><Icon
            style={{ margin: '0.5em' }}
            type='close'
          />
          </span>}
    ...
    renderUpload () {
      const { accept = 'image/*', showHeader = false, saveIntoBusiness = true } = this.props.field;
      return (
        <Upload
          accept={accept}
