...
export default class DeliveryItems extends React.Component {
  ...
  schema = {
    fields: [
      {
        ...
        options: () => {
          ...
          const options = deliveryItems.filter(item => {
            const check = data.indexOf(item.projectCorrelationId)
            return check < 0
          })
          const delivery = options.map(item => ({ key: item.projectCorrelationId, value: item.projectCorrelationId, label: `${item.deliveryName}-${item.categoryName}-${item.goodsName}` }))
          return delivery
        }
      },
      {
    ...
    operations: [{
      ...
      inline: true,
      onClick: (formData) => {
        const { goodsNum, freightPrice, projectCorrelationId: [projectCorrelationId] } = formData
        const delivery = this.props.deliveryItems.find(item => item.projectCorrelationId === projectCorrelationId) || {}
        const receiving = this.props.receivingItems.find(item => item.goodsId === delivery.goodsId)
        const { goodsId, deliveryName, goodsUnit, deliveryAddress, contactName, contactPhone, categoryName, goodsName, goodsUnitCN, deliveryId, firstCategoryId, firstCategoryName } = delivery
        const newItem = {
          ...
          firstCategoryId,
          firstCategoryName,
          correlationObjectId: deliveryId,
          receivingUnit: receiving.goodsUnit,
          goodsNum: (+goodsNum),
    ...
    return (
      <>
        {list.length > 0 && list.map((item, index) => {
          const { projectCorrelationId, deliveryName, deliveryAddress, contactName, contactPhone, categoryName, goodsName, goodsNum, goodsUnitCN, freightPrice } = item
          return (
            <List key={projectCorrelationId}>
              <List.Item extra={<Icon type='cross-circle' onClick={() => this.deleteDelivery(index)} />}><span style={{ fontSize: '17px', color: '#333333' }}>提货点</span></List.Item>
        ...
        {showForm && (
          <MobileForm schema={this.schema} mode={FORM_MODE.ADD}>
            <FormItem fields='projectCorrelationId' />
