...
const { actions: { getTransportAccount } } = model

const Lists = ListContainer(Detail)

@connect(null, { getTransportAccount })
...
export default class Index extends Component{
  ...
  componentDidMount () {
    ...
    getProjectList({ limit: 1000, offset: 0, order: 'desc', isExistAccountTransport: true, isPermissonSelectAll:check||undefined }).then(data => {
      // ES6根据一维对象数组某个属性去重且该属性的值为简单数据类型，比较实用的一种的方法，也基本没有什么性能影响
      this.unique = (arr, key) => {
        const map = new Map()
        return arr.filter((item) => !map.has(`${item[key] }`) && map.set(`${item[key] }`, 1))
      }
      const options = (data.items || []).map(item => ({ key: item.projectId, label: item.projectName, value: item.projectName }))
  ...
  renderList = () => {
    ...
    const check = spacialAuthes.some(auth => ownedPermissions.indexOf(auth) > -1)
    const props = {
      action: this.props.getTransportAccount,
      dataCallBack: this.dataCallBack,
      primaryKey: 'accountTransportId',
      params: {
        accountOrgTypeArr: '1,3',
        projectName: selectedName,
        isSelectPart: !(check||undefined)
      },
      style:{
