        ...
        self.geocoder.getAddress(lnglat, (status, result) => {
          if (status === 'complete' && result.info === 'OK') {
            ...
            result && result.regeocode && result.regeocode.formattedAddress // 是否有获取当前的地理位置
              ? this.props.onMarkerAddress && this.props.onMarkerAddress(lnglat.join(), result.regeocode.formattedAddress)
    ...
    if (!this.hasAddedListener){
      ...
      window.AMap.event.addListener(this.autoCompleteDom, 'select', e => {
        if (e.poi.location) { // 如果存在当前地理位置的坐标
          ...
          this.props.onMarkerAddress && this.props.onMarkerAddress(`${longitude},${latitude}`, district + addressName)
          this.setState({
    ...
    return (
      <>
        ...
        <div style={{ width: '100%', height: '75vh' }}>
          {
            mounted ?
              <Map ref={dom =>this._map = dom} events={this.mapEvents} {...mapConfig} amapkey="f5230ee3a560bd0cb5f5cef45b9c24c8">
                <Marker
