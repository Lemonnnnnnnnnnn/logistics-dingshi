...
import styles from './transport-data-analysis.less';

export default function TransportDataAnalysis (){
  const [ tableData, setTableData ] = useState([]);
  const [ nowPage, setNowPage ] = useState(1);
  const [ filter, setFilter] = useState({ createTime:[moment().startOf('month'), moment().startOf('endOf')] });
  useEffect(()=>{
  ...
  const filterReset = () => {
    const newFilter = {
      offset: 0,
      limit: 10,
      createTime:[moment().startOf('month'), moment().startOf('endOf')]
    };
    setFilter(newFilter);
  ...
  const handleSearchBtnClick = (formData={}, isReset=true) => {
    const createDateStart = formData?.createTime?.[0]?.set({ hour: 0, minute: 0, second: 0, millisecond: 0 }).format('YYYY/MM/DD HH:mm:ss');
    const createDateEnd = formData?.createTime?.[1]?.set({ hour: 23, minute: 59, second: 59, millisecond: 999 }).format('YYYY/MM/DD HH:mm:ss');
