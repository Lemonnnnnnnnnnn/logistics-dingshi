...
import styles from './manual-entry-model.less';
...
export default class ManialEntryModal extends Component {
  ...
  componentDidMount () {
    Promise.all([getAuthorizationPhone(), getOrganizations({ organizationTypeList: '4,5', selectType: '1', offset: 0, limit: 100000 })])
      .then(res => {
        const schema = {
          smsCode: {
            component: GetSmsCode,
            needCheckCode: false,
            callback : ()=>{ this.setState({ sendPassWord : true }); },
            smsType: 'PAY_ORDER',
            rules: {
        ...
        this.setState({
          ...
          schema,
          options : res[1].items.map(item=>({ label : item.organizationName, value : item.organizationId })),
          ready : true
        });
      });
  }
    ...
    <div styleName='manualEntryModal'>
      <SearchForm layout='inline' schema={schema}>
        <div style={{ margin: '0 auto', width: '80%' }}>
          ...
          <Select onChange={this.onSelectOrganization} placeholder='请输入货主名称' style={{ width : '15rem' }} showSearch optionFilterProp='children' filterOption={(input, option)=>option.props.children.toLowerCase().indexOf(input.toLowerCase()) >= 0}>
