...
import EditCarGroup from './component/edit-car-group';
import ChangeCarGroup from './component/change-car-group';
import SelfCarSwitch from "./component/self-car-switch";
  ...
  tableSchema = {
    ...
    operations: (record) => {
      ...
      const modify = {
        title: '编辑',
        onClick: ({ carId }) => {
          router.push(`carManagement/edit?carId=${carId}`);
        }
      };
      ...
      const detail = {
        title: '详情',
        onClick: ({ carId }) => {
          router.push(`carManagement/detail?carId=${carId}`);
        }
      };
