...
import styles from './tax-rate-config-modal.less';
...
const  TaxRateConfigModal = ({
  ...
  const onOk = () => {
    form.validateFields((err, values) => {
      ...
      let endPeriodTime;
      if (values.longTerm) {
        startPeriodTime = values.periodTime;
      } else {
        startPeriodTime = values.periodTime && values.periodTime[0];
        endPeriodTime = values.periodTime && values.periodTime[1];
      }
      if (state.currentId) {
        ...
        <Form>
          ...
          <Form.Item label="应纳所得税">
            ...
            {form.getFieldDecorator(`taxableIncome`, {
              ...
              <Input className="min2-width" placeholder="请输入应纳所得税" disabled={!!state.currentId} />
            )}
          </Form.Item>
          <Form.Item label="速算扣除数（月）">
          ...
          <Form.Item label="有效期限">
            {form.getFieldDecorator(`longTerm`, {
              ...
              <Radio.Group onChange={(e) => {
                if (e.target.value === 0) {
                  form.setFieldsValue({
                    periodTime: periodTime.length ? periodTime : [],
                  });
                } else  {
                  form.setFieldsValue({
