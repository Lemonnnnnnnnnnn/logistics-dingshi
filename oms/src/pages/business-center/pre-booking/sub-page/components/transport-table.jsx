...
import prebookingService, { countRemainWeight } from "../../../../../services/prebooking";
import EditableTable from "../../../../../components/editable-table/editable-table";
import { pick, isEmpty, uniqBy } from "../../../../../utils/utils";
import shipmentCarModel from "../../../../../models/shipmentCars";
import shipmentDriverModel from "../../../../../models/shipmentDrivers";
import DeliveryGoodsInfo from "./components/delivery-goods-info";
...
class TransportTable extends Component {
  ...
  componentDidMount() {
    ...
    if (transportType === 1) {
      Promise.all([getShipmentCars({ limit: 10000, offset: 0 }), getShipmentDrivers({ limit: 1000, offset: 0 })])
        .then(() => {
          // this.props.refreshCount(this.countAgain)
          this.setState({
            ready: true
          });
        });
    } else {
      Promise.all([getPlatformAllCars({ limit: 10000, offset: 0 }), getPlatformAllDrivers({
        limit: 1000,
        offset: 0
      })])
        .then(() => {
      ...
      return (
        <li key={item.goodsId}>
          <span style={numberStyle}>
            {`${item.categoryName}-${item.goodsName}——派车重量:${item.loadingNum.toFixed(2)}${item.goodsUnitCN}/剩余重量：${item.remainingNum.toFixed(2)}${item.goodsUnitCN}`}
          </span>
        </li>);
    });
    return (
  ...
  handleSaveBtnClick = (value) => {
    const { transportType = 1 } = this.props;
    const receiving = this.props.preBooking.receivingItems.find(item => item.receivingId === (+value.receivingId));
