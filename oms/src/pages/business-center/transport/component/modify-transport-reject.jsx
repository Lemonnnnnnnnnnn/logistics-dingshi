...
export default class ModifyTransportReject extends Component {
  ...
  renderForm = () => {
    ...
    const imageData = data[rejectIndex].billDentryid.split(',').map(item => getOssImg(item));
    ...
    const weighForm =
      <SchemaForm mode={FORM_MODE.MODIFY} data={entity} schema={this.weighSchema}>
        ...
        <ImageDetail imageData={data[rejectIndex].weighDentryid && data[rejectIndex].weighDentryid.split(',').map(item => getOssImg(item))} />
