...
import {
  getconTodayAndYesIn,
  getConTranStatusTotal,
  getRroRecivingList,
  getTenderBidderNotify
} from "@/services/apiService";
import { CARGOES, SHIPMENT } from "@/constants/organization/organizationType";
import { isEmpty } from "@/utils/utils";
import OrderAddInfoList from "./components/OrderAddInfoList";
import PreBookingDonut from "./components/PreBookingDonut";
import TransportDonut, { ErrorBoundary } from "./components/TransportDonut";
import InviteTenderModal from "./components/InviteTenderModal";
...
class IndexPageOwnerAndShipment extends React.Component {
  ...
  componentDidMount() {
    if (this.organizationType === CARGOES) { // 如果是货权
      ...
      Promise.all([getconTodayAndYesIn(), getConTranStatusTotal(), getRroRecivingList({
        offset: 0,
        limit: 10,
        isCount: false,
        projectStatus: 1
  ...
  renderOrderStatistics = () => {
    const {
      preToBeDetermined, preDispatchIng, preRefuse, preOver, tranUnRece, tranAccept,
      tranToBePick, tranportIng, tranSign, tranTobeAudit, tranAbnormal, tranReject
    } = this.state.orderInfoData;
    ...
    if (this.state.isIe) {
      return (
        <Card title="单据信息" bordered={false}>
          <h1
            style={{ padding: "120px 0", textAlign: "center", fontSize: "30px", color: "#aaa" }}
          >该图表显示不支持IE10及以下版本
          </h1>
        </Card>
      );
    ...
    return (recivingList || []).map(item => (
        ...
        title={
          <span>
            <span style={{
              fontSize: "14px",
              paddingRight: "5px",
              color: item.isAvailable ? "#00A854" : "#EA5D2A"
            }}
            >{item.isAvailable ? "启用" : "禁用"}
            </span>
            <span style={{ fontSize: "14px", color: "#0D0D0D" }}>{item.projectName}<span
              style={{ fontSize: "10px", color: "#999" }}
            >（{(item.receivingRespList || []).length}个卸货点）
            </span>
            </span>
          </span>
        }
      >
        {(item.receivingRespList || []).map(itm => (
          <Menu.Item
            key={`${item.projectId}-${itm.receivingId}`}
          >{itm.receivingName} （{itm.preBookingTotal}个预约单）
          </Menu.Item>
        ))}
      </SubMenu>
    ));
    ...
    recivingList.some(item => {
      if (item.projectId === +key[0]) {
        showMarkerList = item.receivingRespList.map(itm => {
          ...
          const {
            preBookingTotal = 0, receivingName = "", receivingLatitude,
            receivingLongitude, receivingId
          } = itm;
          return {
            receivingId,
            isAvailable,
  ...
  render() {
    ...
    return (
      <>
        ...
        {this.organizationType === SHIPMENT && <InviteTenderModal />}
        {this.organizationType !== CARGOES && ready &&
        <OrderAddInfoList {...addInfoData} organizationType={this.organizationType} />}
