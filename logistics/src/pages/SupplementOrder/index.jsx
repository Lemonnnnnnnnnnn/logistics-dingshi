...
import { SchemaForm, Item, FormButton, Observer, FORM_MODE } from '@gem-mine/antd-schema-form';
...
class SupplementOrder extends Component {
      ...
      consignmentId: {
        ...
        value : Observer({
          ...
          action : (projectId)=>{
            ...
            const projectDetail = projectList.find(item=>item.projectId === projectId);
            if (!projectDetail){
              return '-';
            }
            const {  consignmentId } = projectDetail;
      ...
      tradingSchemeName: {
        ...
        value : Observer({
          ...
          action : (projectId)=>{
            ...
            const { configurationStatus, tradingSchemeName, projectName, consignmentName } = projectDetail;
            this.projectName = projectName;
            this.consignmentName = consignmentName;

            return configurationStatus === 3 ? tradingSchemeName || '无' : '无需配置';
          }
        })
      },
      businessTypeName: {
        ...
        value : Observer({
          ...
          action : (projectId)=>{
            ...
            const {  businessTypeName, transportBill } = projectDetail;
            this._logisticsBusinessTypeEntity = { transportBill };

            return businessTypeName || '无';
          }
        })
      },
      consignmentName: {
        ...
        value : Observer({
          ...
          action : (projectId)=>{
            ...
            const {  consignmentName } = projectDetail;
      ...
      billMsg: {
        ...
        value : Observer({
          ...
          action : (projectId)=>{
            ...
            const { transportBill, billNumberType, billPictureType } = projectDetail;
            const transportBillList = transportBill ?  transportBill.split(',') : [];
            const billPictureTypeList = billPictureType ?  billPictureType.split(',') : [];
            const billNumberTypeList = billNumberType ?  billNumberType.split(',') : [];
            // 如果单据是自动生成的，补单明细中不能修改相应图片，所以这里要存一份billPictureType
            this.billPictureType = billPictureTypeList;

            return {
              transportBill :transportBillList,
              billPictureType : billPictureTypeList,
              billNumberType  :billNumberTypeList,
            };
          }
        })
      },
  ...
  handleSaveBtnClick = value => {
    const { excelList, excelAddress } = this.state;
    const { projectId, consignmentId, uploadPicture } = value;
