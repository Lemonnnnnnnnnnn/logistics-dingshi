...
import { SchemaForm, FormCard, FORM_MODE, Item, FormButton } from '@gem-mine/antd-schema-form';
...
export default class Authentication extends Component {
  ...
  constructor (props, context) {
    ...
    if (!JSON.parse(localStorage.getItem('token_storage'))) {
      router.push('/user/login');
      return null;
    }
    this.organizationType = JSON.parse(localStorage.getItem('token_storage')).organizationType;
    this.auditStatus = JSON.parse(localStorage.getItem('token_storage')).auditStatus;
    this.organizationId = JSON.parse(localStorage.getItem('token_storage')).organizationId;
    this.formSchema = {
      userName: {
        label: '公司简称',
        rules:{
          required: [true, '请输入登录账号'],
          validator: ({ value }) => {
            const reg = /^[0-9]*$/;
            ...
            {authenticationType
              ...
              : (
                <FormCard colCount='1' title='企业信息'>
                  ...
                  <Row>
                    <Col span={10} style={{ paddingRight: '20px' }}>{this.addCorporationDentryid(applicationType)}</Col>
