...
import { SchemaForm, Item, FormButton, Observer } from '@gem-mine/antd-schema-form';
...
import BranchBanksSelector from './BranchBanksSelector';
              ...
              {getFieldDecorator('money', {
                rules: [
                  ...
                  { validator: (rule, value, callback) => {
                    if (value > Number(maxMoney || 0)) callback('超出可提现金额');
                    if (value && Number(value) === 0) callback('金额必须大于0');
                    callback();
                  } }
                ],
              })(
                <Input size="large" placeholder={`可提现金额${Number(maxMoney || 0).toFixed(2)._toFixed(2)}元`} addonAfter={<div style={{ cursor: 'pointer' }} onClick={this.allIn}>全部</div>} />
              )}
            </Form.Item>
          </div>
          <div styleName='item' style={{ marginTop: '25px' }}>
            <p>提现原因<span styleName='required'>(必填)</span></p>
            <Form.Item styleName='item'>
              {getFieldDecorator('remarks', {
                rules: [{ required: true, message: '请输入提现原因' }],
              })(
                <TextArea placeholder='请输入提现原因' maxLength={100} rows={3} />
              )}
            </Form.Item>
          </div>
          <footer styleName='footer_tips'>
            <p>温馨提示：</p>
            <p>1.提现申请成功后，等待处理时间预计为48小时，如金额错误可到收支记录撤销后重新申请</p>
            <p>2.如平台超过48小时未处理该提现申请将关闭，如需提现请重新提交申请</p>
          </footer>
