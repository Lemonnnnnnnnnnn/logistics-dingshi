...
import { Popover, Icon, Modal, Input, Button, notification, message } from 'antd';
...
import {
  postBussinessBehalfPay,
  postTransferBehalfPay,
  postScanPayOrderPay,
  repayManualEntry,
  sendFinanceBusinessExcelPost,
  deleteCustomerRelationships,
  exportFinanceBusinessPayOrderPdf, sendFinanceAccountsExcelPost
} from "@/services/apiService";
import { translatePageType, pick, routerToExportPage, getLocal, getOssImg } from '@/utils/utils';
...
class BusinessDealings extends Component {
        ...
        render :(text, record)=> (
          <div>
            ...
            <Authorized authority={[FUNDS_BUSINESS_DEALINGS_REPAY]}>
              <a style={{ marginRight : '1em' }} onClick={this.repay(record)}>重新支付</a>
            </Authorized>
            }
            {
              (record.businessType === 7 || record.businessType === 20 || record.businessType === 27) && record.orderPayDentryid &&
              // <Authorized authority={[FUNDS_BUSINESS_DEALINGS_REPAY]}>
              <a onClick={()=>this.onViewCertificate(record)}>查看支付凭证</a>
              // </Authorized>
            }
          </div>)
      },
  ...
  componentDidMount() {
    ...
    const params = {
      ...
      payDateStart: localData.formData.payTime && localData.formData.payTime.length ? moment(localData.formData.payTime[0]).startOf('day').format('YYYY/MM/DD HH:mm:ss') : undefined,
      payDateEnd: localData.formData.payTime && localData.formData.payTime.length ? moment(localData.formData.payTime[1]).endOf('day').format('YYYY/MM/DD HH:mm:ss') : undefined,
  ...
  onViewCertificate = record => {
    const { orderPayDentryid } = record;
    window.open(getOssImg(orderPayDentryid));
  }

  exportCertificatePdf = () =>{
    const { selectedRow } = this.state;
    const { filter, financeBusiness } = this.props;

    if (!financeBusiness?.count){
      return message.error('当前列表并没有可导出的的订单');
    }
    ...
    const payDateStart = filter.payTime && filter.payTime.length ? moment(filter.payTime[0]).startOf('day').format('YYYY/MM/DD HH:mm:ss') : undefined;
    const payDateEnd = filter.payTime && filter.payTime.length ? moment(filter.payTime[1]).endOf('day').format('YYYY/MM/DD HH:mm:ss') : undefined;
    const transactionAmountMin = filter.transactionAmount?.[0];
    const transactionAmountMax = filter.transactionAmount?.[1];
    let newFilter = {
      ...filter,
      offset: 0,
      limit : 100000,
    ...
    if (selectedRow.length){
      // const filterSelectedRow = selectedRow.filter(item=>item.orderPayDentryid);
      // if (!filterSelectedRow.length) {
      //   return message.error("请选择存在支付凭证的订单");
      // }

      newFilter = { ...newFilter, financeBusinessIdList : selectedRow.map(item=>item.financeBusinessId).join(',') };
    }


    routerToExportPage(exportFinanceBusinessPayOrderPdf, newFilter);
  }
      ...
      <SearchForm layout='inline' {...formLayOut} schema={this.searchSchema} mode={FORM_MODE.SEARCH}>
        ...
        <div style={{ margin : '1rem 0' }}>
          ...
          <Authorized authority={[FUNDS_BUSINESS_DEALINGS_EXPORT_DETAIL]}>
            <Button className='mr-10' onClick={this.getExcel}>导出明细</Button>
          </Authorized>
          <Button onClick={this.exportCertificatePdf}>导出支付凭证PDF</Button>
        </div>
      </SearchForm>
    );
  }
  ...
  handleSearchBtnClick = value => {
    ...
    const payDateStart = value.payTime && value.payTime.length ? moment(value.payTime[0]).startOf('day').format('YYYY/MM/DD HH:mm:ss') : undefined;
    const payDateEnd = value.payTime && value.payTime.length ? moment(value.payTime[1]).endOf('day').format('YYYY/MM/DD HH:mm:ss') : undefined;
