...
import { Form, Input, Button, Radio, message, Modal, Checkbox, Icon } from 'antd';
...
import UserAgreement from '../BusinessCenter/ProjectManagement/subPage/userAgreement';
...
export default class Login extends Component {
  ...
  componentDidMount () {
    const { location, fetchCurrent, clearSetTabs } = this.props;
  ...
  login = (event) => {
    ...
    localStorage.removeItem("ejd-showHomeModal");
    needSlideValidate
    ...
    validateFields((err, values) => {
      ...
      login({ ...values, ...extraValue, scope: SCOPE[values.roleType || role.PLATFORM], password: encodePassword(values.password) })// 要传入角色权限scope详情见接口
        ...
        .then(() => {
          ...
          if (location && location.query.redirect) {
      ...
      <div className={styles.loginBlock}>
        ...
        <Form onSubmit={this.login} className={styles.loginForm}>
          ...
          <div style={{ margin: '20px 5px 5px', display: 'flex', justifyContent: 'space-between' }}>
            ...
            >
              <span>阅读并同意</span>
              <a href='/userAgreement' style={{ color:'#14428A' }}>《用户使用协议》</a>
            </Checkbox>
            {!isIndepend && <div style={{ textAlign: 'right', color: '#14428A' }} onClick={this.toModifyPassword}>重置密码</div>}
          </div>
          }
        </Form>
      </div>
