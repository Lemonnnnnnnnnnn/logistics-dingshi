...
export default class ReceivingSetting extends Component{
  ...
  formSchema = {
    ...
    receivingLabelId: {
      ...
      value: Observer({
        ...
        action : (value, d)=> {
          if (value && d.changeValues && !d.changeValues.receivingAddress) {
    ...
    customerOrgId: {
      ...
      component: 'select',
      options: () => {
        return this.state.options1.map(item=>({
  ...
  onOk = (data) => () => {
    ...
    this.form2.setFieldsValue({
      customerOrgId: this.state.options1.some(item => item.organizationId === current.customerOrgId) ? current.customerOrgId : current.customerOrgName,
      contactPhone: current.contactPhone, contactName: current.contactName,
    });
  }
  ...
  saveData = formData => {
    const submitValue = { ...formData, ...formData.receivingAddressObject, receivingAddress: formData.receivingAddress, areaCode: formData.receivingAddressObject.receivingAdCode };
    delete submitValue.receivingAddressObject;
    const { mode, options1, options } = this.state;
    const { postReceiving, patchReceiving, getReceiving, setFilter, filter } = this.props;
    const customerOrgId = isNaN(formData.customerOrgId) ? options.find(item => item.customerOrgName === formData.customerOrgId).customerOrgId : formData.customerOrgId;
    mode===FORM_MODE.ADD
      ?postReceiving({ ...submitValue, signDentryid: formData.signDentryid.shift(), customerOrgId })
        .then(()=>{
      ...
      :patchReceiving({ ...submitValue, signDentryid: formData.signDentryid.shift(), receivingId: this.props.entity.receivingId, customerOrgId: options1.some(item => item.organizationId === submitValue.customerOrgId) ? customerOrgId :  this.props.entity.customerOrgId })
        .then(() => {
          notification.success({
            message: '修改成功',
            description: `修改卸货点成功`,
  ...
  render (){
    ...
    const { mode, nowPage, pageSize, ready, visible, isUsed, options1 } = this.state;

    const detail = mode === FORM_MODE.ADD
      ? null
      : { ...entity, customerOrgId: options1.some(item => item.organizationId === entity.customerOrgId) ? entity.customerOrgId : entity.customerOrgName };
    ...
    return (
      <>
        ...
        <Modal
          ...
          width={550}
        >
          <SchemaForm layout="vertical" mode={mode} data={{ ...detail }} schema={this.formSchema} trigger={{ isUsed }}>
            <Item field="receivingName" />
            <Item field="receivingLabelId" />
