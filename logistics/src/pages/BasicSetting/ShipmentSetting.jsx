...
import { Button, Row, Col, Input, Modal, List, Card, notification, Select } from "antd";
      ...
      <>
        ...
        >
          ...
          <Row type='flex' justify='center' align='middle' style={{ height : '5rem' }}>
            ...
            >
              {
                shipmentItems.map(item=><Select.Option key={item.shipmentId} value={item.shipmentId}>{item.title}</Select.Option>)
              }
            </Select>
          </Row>

        </Modal>
        <Table schema={this.tableSchema} rowKey="organizationId" renderCommonOperate={this.searchTableList} pagination={{ current: this.state.nowPage, pageSize:this.state.pageSize }} onChange={this.onChange} dataSource={shipment} />
      </>
    );
  }
}
