...
import { Row, Col, Icon, Checkbox, notification, Tooltip, Timeline, Button } from 'antd';
import { getElectronicDocuments, getGoodsCategories } from '@/services/apiService';
import styles from './BusinessTypeSetting.less';
import DebounceFormButton from '@/components/DebounceFormButton';
import CheckBox from '../component/CheckBox';
import LinkageCheckBox from '../component/LinkageCheckBox';
import SelectWithPicture from '../component/SelectWithPicture';
import model from '@/models/businessTypes';
import { pick, intersection, getLocal, isEmpty } from '@/utils/utils';
import moreIcon from '@/assets/more.png';

const {
  actions: { getBusinessTypes, postBusinessTypes },
} = model;
...
@connect(
  mapStateToProps,
  { getBusinessTypes, postBusinessTypes }
)
@CSSModules(styles, { allowMultiple: true })
export default class BusinessTypeSetting extends React.Component {
  currentTab = this.props.commonStore.tabs.find(
    item => item.id === this.props.commonStore.activeKey
  );
  ...
  formSchema = {
    ...
    executeReceiptOvertime: {
      label: '已接单运单长时间未提货是否自动关闭：',
      component: 'radio',
      rules: {
        required: [true, '请选择已接单运单长时间未提货是否自动关闭'],
      },
      options: [
        {
          label: '不自动关闭',
    ...
    transportCreateMode: {
      ...
      options: [
        {
          label: "电子围栏自动触发",
          // label: <div style={{ display: "flex" }}>电子围栏自动触发  <Tooltip placement="topLeft" title="当勾选”轨迹跟踪记录“选项时，同时项目里配置了“自动调度信息”时，相关车辆形成的轨迹记录可以自动产生预约单和运单。"><Icon type="question-circle" style={{ color: "#1890FF" }} /></Tooltip></div>,
          key: 1,
          value: 1,
        },
        // {
        //   label:  <div style={{ display: "flex" }}>收件地址（外部系统对接） <Tooltip placement="topLeft" title="当勾选”收货地址（外部系统对接）选项时，才可以通过接口从外部系统对接产生预约单、运单。"><Icon type="question-circle" style={{ color: "#1890FF" }} /></Tooltip></div>,
        //   key: 2,
        //   value: 2,
        // }
      ],
    ...
    deliveryElectronicDocumentsId: {
      ...
      options: async () => {
        const { items } = await getElectronicDocuments({
          electronicDocumentsType: 1,
          limit: 1000,
          offset: 0,
        });
        return [
    ...
    //   }),
    //   rules: {
    //     required: [true, '请选择业务类别：'],
    //   },
    //   options: async () => {
    //     const { items } = await getGoodsCategories({ parentId : 0 });
  ...
  handleSaveBtnClick = data => {
    ...
    // formData.firstCategoryId = data.firstCategoryId;
    formData.transportCreateMode = data.transportCreateMode;
    ...
    formData.electronicDocumentsIdList = [
      data.deliveryElectronicDocumentsId,
      data.receivingElectronicDocumentsId,
    ].filter(value => value && value !== 'temp');
    ...
    formData.executeReceiptOvertime = data.executeReceiptOvertime;
  ...
  componentWillUnmount() {
    if (this.state.formMode !== FORM_MODE.DETAIL) {
      ...
      if (getLocal('local_commonStore_tabsObj').tabs.length > 1) {
        localStorage.setItem(
          ...
          JSON.stringify({
            formData: {
              ...formData,
              projectDentryid: (formData && formData.projectDentryid) || '',
            },
          })
        );
      }
        ...
        <ul styleName="ul_tabs_container">
          {lists.items.map((item, index) => (
            ...
            <li
              ...
              styleName={tab === item.businessTypeId ? 'active_li' : ''}
  ...
  getActiveData = () => {
    ...
    formData.poundDifference = Number(active.poundDifference * 1000)
      .toFixed(2)
      ._toFixed(2);
    formData.confirmOrganization = Number(active.confirmOrganization);
    formData.deliveryElectronicDocumentsId = (
      (active.electronicDocumentsEntities || []).find(item => item.electronicDocumentsType === 1) ||
      {}
    ).electronicDocumentsId;
    formData.receivingElectronicDocumentsId = (
      (active.electronicDocumentsEntities || []).find(item => item.electronicDocumentsType === 3) ||
      {}
    ).electronicDocumentsId;
  ...
  render() {
    const { ready, formMode, more } = this.state;
    const { lists } = this.props;
    const formLayout = {
      labelCol: {
        span: 24,
              ...
              lists.count > 0 || formMode === FORM_MODE.ADD?
                <>
                  ...
                  <div styleName="item_box">
                    <Row>
                      {/* <Col span={8}>
                        <Item wrapperCol={{ span: 12 }} field="firstCategoryId" />
                      </Col> */}
                      <Col span={8}>
                        <Item wrapperCol={{ span: 12 }} field="businessTypeName" />
                      </Col>
                      <Col span={8}>
                        <Item wrapperCol={{ span: 20 }} field="remarks" />
                      </Col>
                    </Row>
                  </div>
                  <h3 styleName="form_title">运单生成方式
                  {/* <Tooltip placement="topLeft" title="无论勾选或不勾选任何一项数据来源都可以手动产生预约单和运单（按项目->预约单->运单）"><Icon type="question-circle" style={{ color: "#1890FF" }} /></Tooltip> */}
                  </h3>
                  <div styleName="item_box">
                    <Item styleName="flex_align_start" field="transportCreateMode" />
                    <br />
                  </div>
                  ...
                  <div styleName="item_box">
                    ...
                    <Row>
                      ...
                      <Col span={7} style={{ width: 'auto' }}>
                        <Item
                          className="businessType_setting_form_releaseHall"
                          field="driverAcceptAudit"
                        />
                      </Col>
                      <Col span={7}>
                  ...
                  <div styleName="item_box">
                    ...
                    <p styleName="mar0">
                      （当司机提货后需要托运审核，则运单状态从待提货状态变为提货审核，托运审核通过后，状态变为运输中；
                    </p>
                    ...
                    <Row>
                      <Col span={6} styleName="autoWidth">
                        <Item
                          className="businessType_setting_form_transportCompleteType"
                          field="transportCompleteType"
                        />
                      </Col>
                      <Col span={18}>
                        <Row>
                    ...
                    <Item styleName="flex_align_start" field="executeReceiptOvertime" />
                  </div>
                  ...
                  {/* <h3 styleName="form_title">模板修改事件</h3>
                  <div>
                    <Timeline>
