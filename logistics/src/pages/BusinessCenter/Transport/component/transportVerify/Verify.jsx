...
import MultipleCheckButton from '@/components/MultipleCheckButton';
import { getTransport, setTransportStatus } from '@/services/apiService';
import { getRole } from '@/services/user';
import { SHIPMENT } from '@/constants/organization/organizationType';
            ...
            <span>
              <ul>
                {this.transportDetail.deliveryItems?.map((item, index) =>
                  ...
                  >
                    <span style={{ flex: 0, marginRight: '0', textAlign: 'left' }}>{`${item.categoryName}-${item.goodsName}`}</span>
          ...
          <p className={styles.infoItem}>
            ...
            <span>
              {this.transportDetail.deliveryItems?.map(item =>
                ...
                >
                  <span style={{ flex: 0, marginRight: '0', textAlign: 'left' }}>{`${item.categoryName}-${item.goodsName}`}</span>
                  <span style={{ textAlign: 'right' }}>{item.receivingNum}{item.receivingUnitCN}(磅差：{calculatePounddifference(item)}‰)</span>
                </li>)}
            </span>
          </p>
          <p className={styles.infoItem}>
      ...
      <>
        <SchemaForm schema={this.schema} data={verifyItem} mode={FORM_MODE.MODIFY} trigger={{ verifyIndex }}>
          ...
          <div style={{ textAlign: 'right' }}>
            {
              verifyIndex > 0 && <DebounceFormButton label='上一步' validate onClick={this.pre} style={{ marginRight: 10 }} />
            }
            {
...
function getImages(items, type) {
  switch (type){
    case 'delivery': return items?.billDentryid ? items.billDentryid.split(',') : [];
    case 'weigh' : return items?.weighDentryid ? items.weighDentryid.split(',') : [];
    case 'receiving' : return items?.billDentryid ? items.billDentryid.split(',') : [];
    default : return [];
  }
}
