...
import { omit, getGoodsName } from '@/utils/utils';
import styles from './DeliveryGoodsInfo.less';
...
class DeliveryGoodsInfo extends Component {
  ...
  renderDelivery = (record, form) => {
    ...
    const deliveryOptions = options.map((item) =>
      <Select.Option key={item.projectCorrelationId} value={item.projectCorrelationId}>{`${item.deliveryName}-${getGoodsName(item)}`}</Select.Option>);
  ...
  render () {
    const tableSchema = [
      {
        ...
        width: '450px',
        render: (text, record) => `${record.deliveryName}-${getGoodsName(record)}`
      }, {
        title: '提货地址',
    ...
    return (
      <div className={styles.DeliveryGoodsInfo}>
        <EditableTable
