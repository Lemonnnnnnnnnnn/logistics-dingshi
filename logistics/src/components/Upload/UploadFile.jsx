...
export default class UploadFile extends React.Component {
  ...
  removeFile = (file, index) => {
    ...
    const { onChange, removeFile  } = this.props;
  ...
  getOriginalName (nameWithKey, renderMode, i) {
    if (renderMode === 'doc' || renderMode === 'pdf&img') {
      ...
      return this.state.oldNames[i] ? this.state.oldNames[i] : fileName;
    }
    return nameWithKey.replace(/_KEY_[^.]+/, '');
  }

  wrappedOnUpload = fileObject => {
    const { files = [], showFiles, oldNames } = this.state;
    const { onChange, afterUpload = null, form, formData } = this.props;
    const { showFileName, fileName, oldName } = fileObject;
    const nextFiles = [...files, fileName];
    const nextShowFiles = [...showFiles, showFileName];
    const nextOldNames = [...oldNames, oldName];
    if (afterUpload) {
      afterUpload(nextFiles, { form, formData });
    }
    this.setState({ files: nextFiles, showFiles: nextShowFiles,  oldNames: nextOldNames }, ()=>{
  ...
  renderUploadedFile = (showFile, index) => {
    ...
    if (renderMode === 'doc' || renderMode === 'pdf&img') {
      return <Tag key={showFile} closable={!readOnly} onClose={() => this.removeFile(showFile, index)}>{this.getOriginalName(showFile, renderMode, index)} {readOnly && <Icon type="search" />}</Tag>;
    }
  ...
  renderImgPreview = (file, index) => {
    ...
    const { needAuthority, allow, afterUpload } = this.props;
    return (
      <span key={file} styleName="img-preview-wrap">
        {
          afterUpload ?
            <PopUpImg type="typeKey" src={imgUrl} largeSrc={largeSrc} needAuthority={needAuthority} allow={allow} afterUpload={afterUpload} />
            :
