...
const RenderSearchInput = ({ searchList = [], form, buttonList, searchObj = {}, pagination = {} }) => {
  // useEffect(() => {
  //   if (searchObj) {
  ...
  const onClick = (item) => () => {
    ...
    form.validateFields((err, values) => {
      item.onClick(values, pagination);
    });
  };
  ...
  const adjustValue = (key) => {
    if (!form.getFieldValue(key) || !form.getFieldValue(key)[0]) return;
    form.setFieldsValue({ [key]: [form.getFieldValue(key)[0].startOf('month'), form.getFieldValue(key)[1].endOf('month')] });
  };
