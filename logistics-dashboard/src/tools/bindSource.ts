...
function bindSource(...params) {
  ...
  return ({
    ...
    const finalEffects = getEffectsInjectRequests(
      { ...sources.effects, ...modelEffects },
      sources.requests,
    );
...
function createSource(partialSource) {
  ...
  const effects = createEffects(nameSpace);
...
function createReduces(namespace, rowKey) {
  const reducers = [
    ...
    {
      type: 'put',
      reduce: _createReduce((payload, state) =>
        replaceTargetItemInList(payload, state.items, rowKey),
      ),
    },
    {
      type: 'patch',
    ...
    {
      type: 'detail',
      reduce: _createReduce((payload) => ({ entity: payload })),
    },
    {
      type: 'setFilter',
      reduce: _createReduce((payload) => ({ filter: payload })),
    },
    {
      type: 'mergeFilter',
      reduce: _createReduce((payload, state) => ({
...
function getRowKey(namespace) {
  ...
  const name = (namespace.name || namespace).replace(
    /(s|es|ies)$/,
    (match) => ({ es: '', s: '', ies: 'y' }[match]),
  );
...
function _createReduce(getPartialState) {
  return function create(state, { payload, type }) {
    return {
      ...state,
...
function schemaItems({ count = 0, ...lastPayload } = {}) {
  const itemsKey = Object.keys(lastPayload).find((key) =>
    isArray(lastPayload[key]),
  );
...
function createRequest(namespace) {
  ...
  const nodes = [
    {
      ...
      method: (params, options) =>
        sendRequest('get', replaceId(urls.get, params), params, options),
    }, //  request.get(replaceId(urls.get, params), { params })
    {
      type: 'post',
      method: (data, options) =>
        sendRequest('post', replaceId(urls.post, data), data, options),
    }, //  request.post(replaceId(urls.post, data), { data })
    {
      type: 'detail',
      method: (params, options) =>
        sendRequest('get', replaceId(urls.detail, params), params, options),
    }, // request.get(replaceId(urls.detail, params), { params })
    {
      type: 'put',
      method: (data, options) =>
        sendRequest('put', replaceId(urls.put, data), data, options),
    }, // request.put(replaceId(urls.put, data), { data })
    {
      type: 'patch',
      method: (data, options) =>
        sendRequest('patch', replaceId(urls.patch, data), data, options),
    }, // request.patch(replaceId(urls.patch, data), { data })
    {
      type: 'delete',
      method: (params, options) =>
        sendRequest('delete', replaceId(urls.delete, params), params, options),
    }, // request.delete(replaceId(urls.delete, params))
  ];
  ...
  const defaultRequests = nodes.reduce((requests, node) => {
    ...
    return {
      ...requests,
      [requestName]: (params, options) =>
        request(params, options).then((data) => {
          const schemaMethod = dataSchema[node.type];
          return typeof schemaMethod === 'function' ? schemaMethod(data) : data;
        }),
    };
  }, requestsFromParams);
...
function sendRequest(method = 'get', url, params, options: obj<any> = {}) {
...
function replaceId(url, params) {
  return url.replace(/\{:([^}]+)\}/, (_, group) => params[group]);
}
